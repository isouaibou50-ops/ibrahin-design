...
// NOTE: This is a mock server-side Route Handler for Next.js (app router).
// It uses an in-memory store (for demo) — replace with your DB (Mongo / Postgres) in production.

// import { NextResponse } from 'next/server';

// // Simple in-memory "DB" (module-scoped so it survives during a single server run)
// const ORDERS = global.__IBRAHIM_ORDERS__ || [];
// if (!global.__IBRAHIM_ORDERS__) global.__IBRAHIM_ORDERS__ = ORDERS;
...
//       status: 'delivered',
//       items: [{ productId: 'p_1', name: 'Kente Shirt', qty: 1, price: 499 }],
//       total: 499,
...
//       },
//       images: [],
//       notes: 'Make the collar slightly taller.',
//       priceEstimate: 1200,
//     }
//   );
// }
...
//   // Optional: read userId from query or header for demo
//   const url = new URL(request.url);
//   const userId = url.searchParams.get('userId') || 'user_demo_1';
//   const page = Number(url.searchParams.get('page') || 1);
//   const limit = Number(url.searchParams.get('limit') || 20);

//   const userOrders = ORDERS.filter((o) => o.userId === userId).sort((a, b) => b.createdAt - a.createdAt);
//   const start = (page - 1) * limit;
//   const paged = userOrders.slice(start, start + limit);

//   return NextResponse.json({ success: true, orders: paged, meta: { total: userOrders.length, page, limit } });
// }

// export async function POST(request) {
//   try {
//     const body = await request.json();
//     const userId = body.userId || 'user_demo_1';
...
//       images: body.images || [], // expect base64 strings (demo)
//       notes: body.notes || '',
//       priceEstimate: body.priceEstimate || null,
//       tracking: [{ ts: Date.now(), status: 'created', note: 'Order created' }],
//     };
...
//   } catch (err) {
//     console.error('POST /api/user/orders error', err);
//     return NextResponse.json({ success: false, message: err.message || 'Server error' }, { status: 500 });
//   }
// }


// // File: app/api/user/orders/[id]/route.js
// import { NextResponse } from 'next/server';

// const ORDERS_REF = global.__IBRAHIM_ORDERS__ || [];

// export async function GET(request, { params }) {
//   const { id } = params;
//   const order = ORDERS_REF.find((o) => o.id === id);
...
//     const { id } = params;
//     const body = await request.json();
//     const idx = ORDERS_REF.findIndex((o) => o.id === id);
//     if (idx === -1) return NextResponse.json({ success: false, message: 'Not found' }, { status: 404 });

//     const current = ORDERS_REF[idx];

//     // allow updating status, notes, measurements, images
//     if (body.status) {
//       current.status = body.status;
//       current.tracking = current.tracking || [];
//       current.tracking.push({ ts: Date.now(), status: body.status, note: body.note || 'Status updated' });
//     }
//     if (body.notes !== undefined) current.notes = body.notes;
//     if (body.measurements) current.measurements = { ...(current.measurements || {}), ...body.measurements };
//     if (Array.isArray(body.images) && body.images.length > 0) {
//       // merge images (demo: append)
//       current.images = [...(current.images || []), ...body.images];
//     }

//     ORDERS_REF[idx] = current;
//     return NextResponse.json({ success: true, order: current });
//   } catch (err) {
//     console.error('PATCH /api/user/orders/[id] error', err);
//     return NextResponse.json({ success: false, message: err.message || 'Server error' }, { status: 500 });
//   }
// }
...
//   const idx = ORDERS_REF.findIndex((o) => o.id === id);
//   if (idx === -1) return NextResponse.json({ success: false, message: 'Not found' }, { status: 404 });
//   ORDERS_REF.splice(idx, 1);
//   return NextResponse.json({ success: true });
// }


// // File: components/UserDashboard.jsx
// // Mobile-first, production-ready UI for user order dashboard (client component)
// // Usage: import UserDashboard from '@/components/UserDashboard' and render on /dashboard
